...
static ssize_t fsize_checker_post_count_cfn(void * user_data, char ** restrict buf, size_t * restrict buf_size)
{
...
int main(int argc, char * argv[])
{
    ...
    if (argc == 7) {
        ...
        if (!qn_rdr_add_post_filter(ctrl_rdr, &fc, fsize_checker_post_count_cfn)) {
        ...
        if (!qn_rdr_add_post_filter(ctrl_rdr, etag, qn_flt_etag_update_cfn)) {
