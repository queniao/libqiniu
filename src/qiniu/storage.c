...
typedef struct _QN_STORAGE
{
    ...
    unsigned int up_entry_idx:4;
    unsigned int up_entry_cnt:4;
    unsigned int io_entry_idx:4;
    unsigned int io_entry_cnt:4;
    unsigned int rs_entry_idx:4;
    unsigned int rs_entry_cnt:4;
    unsigned int rsf_entry_idx:4;
    unsigned int rsf_entry_cnt:4;
} qn_storage;
...
QN_SDK qn_storage_ptr qn_stor_create(qn_region_ptr restrict rgn)
{
    ...
    new_stor->up_entry_idx = 0;
    count = qn_rgn_host_entry_count(qn_rgn_get_up_host(new_stor->rgn));
    new_stor->up_entry_cnt = count <= 16 ? count : 16;

    new_stor->io_entry_idx = 0;
    count = qn_rgn_host_entry_count(qn_rgn_get_io_host(new_stor->rgn));
    new_stor->io_entry_cnt = count <= 16 ? count : 16;

    new_stor->rs_entry_idx = 0;
    count = qn_rgn_host_entry_count(qn_rgn_get_rs_host(new_stor->rgn));
    new_stor->rs_entry_cnt = count <= 16 ? count : 16;

    new_stor->rsf_entry_idx = 0;
    count = qn_rgn_host_entry_count(qn_rgn_get_rsf_host(new_stor->rgn));
    new_stor->rsf_entry_cnt = count <= 16 ? count : 16;
...
QN_SDK qn_json_object_ptr qn_stor_mn_api_stat(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict bucket, const char * restrict key, qn_stor_management_extra_ptr restrict mne)
{
    ...
    if (! (rgn_entry = qn_rgn_host_get_entry(qn_rgn_get_rs_host(stor->rgn), stor->rs_entry_idx))) {
    ...
    qn_str_destroy(url);
    if (! ret) {
        if (qn_err_3rdp_is_curl_easy_error_occurred()) {
            if (++stor->rs_entry_idx == stor->rs_entry_cnt) stor->rs_entry_idx = 0;
        } // if
        return NULL;
    } // if
...
QN_SDK qn_json_object_ptr qn_stor_ls_api_list(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict bucket, qn_stor_list_extra_ptr restrict lse)
{
    ...
    // ---- Process all extra options.
    if (! (rgn_entry = qn_rgn_host_get_entry(qn_rgn_get_rsf_host(stor->rgn), stor->rsf_entry_idx))) {
        qn_err_stor_set_lack_of_region_entry();
        return NULL;
    } // if
    ...
    if (! ret) {
        if (qn_err_3rdp_is_curl_easy_error_occurred()) {
            if (++stor->rsf_entry_idx == stor->rsf_entry_cnt) stor->rsf_entry_idx = 0;
        } // if
        return NULL;
    } // if
...
QN_SDK qn_json_object_ptr qn_stor_ft_api_fetch(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict src_url, const char * restrict dest_bucket, const char * restrict dest_key, qn_stor_fetch_extra_ptr restrict fte)
{
    ...
    // ---- Process all extra options.
    if (! (rgn_entry = qn_rgn_host_get_entry(qn_rgn_get_io_host(stor->rgn), stor->io_entry_idx))) {
        qn_err_stor_set_lack_of_region_entry();
        return NULL;
    } // if
    ...
    if (! ret) {
        if (qn_err_3rdp_is_curl_easy_error_occurred()) {
            if (++stor->io_entry_idx == stor->io_entry_cnt) stor->io_entry_idx = 0;
        } // if
        return NULL;
    } // if
...
QN_SDK qn_json_object_ptr qn_stor_up_api_upload_file(qn_storage_ptr restrict stor, const char * restrict uptoken, const char * restrict fname, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    if (! (rgn_entry = qn_rgn_host_get_entry(qn_rgn_get_up_host(stor->rgn), stor->up_entry_idx))) {
        qn_err_stor_set_lack_of_region_entry();
        return NULL;
    } // if
    ...
    if (! ret) {
        if (qn_err_3rdp_is_curl_easy_error_occurred()) {
            if (++stor->up_entry_idx == stor->up_entry_cnt) stor->up_entry_idx = 0;
        } // if
        return NULL;
    } // if
...
static size_t qn_stor_upload_cfn(void * user_data, char * buf, size_t size)
{
...
QN_SDK qn_json_object_ptr qn_stor_up_api_upload(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict data_rdr, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    ret = qn_http_form_add_file_reader(qn_http_req_get_form(stor->req), "file", qn_str_cstr(qn_io_rdr_name(data_rdr)), NULL, qn_io_rdr_size(data_rdr), mime_type, stor->req);
    if (! ret) return NULL;

    qn_http_req_set_body_reader(stor->req, data_rdr, qn_stor_upload_cfn, qn_io_rdr_size(data_rdr));
...
QN_SDK qn_stor_resumable_upload_ptr qn_stor_ru_create(qn_io_reader_itf restrict data_rdr)
{
    ...
    ru->fsize = qn_io_rdr_size(data_rdr);
    ...
    ru->src_rdr = qn_io_rdr_duplicate(data_rdr);
    if (! ru->src_rdr) {
        qn_json_destroy_object(ru->progress);
        free(ru);
        return NULL;
...
QN_SDK qn_bool qn_stor_ru_attach(qn_stor_resumable_upload_ptr restrict ru, qn_io_reader_itf restrict data_rdr)
{
    ...
    new_rdr = qn_io_rdr_duplicate(data_rdr);
...
QN_SDK qn_io_reader_itf qn_stor_ru_create_block_reader(qn_stor_resumable_upload_ptr restrict ru, int blk_idx, qn_json_object_ptr * restrict blk_info)
{
    ...
    if (ru->blk_cnt == 0) {
        ...
        *blk_info = new_blk_info;
        return qn_io_rdr_section(ru->src_rdr, 0, blk_size);
    } // if
    ...
    *blk_info = new_blk_info;
    return qn_io_rdr_section(ru->src_rdr, blk_idx * QN_STOR_RU_BLOCK_MAX_SIZE, blk_size);
}
...
static qn_bool qn_stor_ru_prepare_for_resumable_upload(qn_storage_ptr restrict stor, const char * restrict uptoken, const char * restrict mime, qn_io_reader_itf restrict rdr, qn_uint size, qn_rgn_entry_ptr rgn_entry)
{
    ...
    if (rgn_entry && rgn_entry->hostname && ! qn_http_req_set_header(stor->req, "Host", qn_str_cstr(rgn_entry->hostname))) return qn_false;
    ...
    qn_http_req_set_body_reader(stor->req, rdr, qn_stor_upload_cfn, size);
...
QN_SDK qn_json_object_ptr qn_stor_ru_api_bput(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict data_rdr, qn_json_object_ptr restrict blk_info, qn_uint chk_size, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    if (! qn_stor_ru_prepare_for_resumable_upload(stor, uptoken, "application/octet-stream", data_rdr, chk_size, NULL)) return NULL;
...
QN_SDK qn_json_object_ptr qn_stor_ru_api_mkfile(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict ctx_rdr, qn_json_object_ptr restrict last_blk_info, qn_fsize fsize, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    if (! qn_stor_ru_prepare_for_resumable_upload(stor, uptoken, "text/plain", ctx_rdr, ctx_size, NULL)) return NULL;
...
QN_SDK qn_json_object_ptr qn_stor_ru_upload_huge(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_stor_resumable_upload_ptr ru, int * start_idx, qn_uint chk_size, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    for (i = *start_idx; i < qn_stor_ru_get_block_count(ru); i += 1) {
        ...
        } else if (! qn_io_rdr_advance(sec_rdr, offset)) {
            goto QN_STOR_UPLOAD_HUGE_ERROR_HANDLING;
        } // if
