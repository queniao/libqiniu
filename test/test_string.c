...
void test_join(void)
{
    ...
    str1 = qn_str_join(",", srcs[0], QN_STR_ARG_END);
    if (!str1) {
        CU_FAIL("Cannot join one string to a new one.");
        return;
    } // if
    ...
    str2 = qn_str_join("::", srcs[0], srcs[1], QN_STR_ARG_END);
    if (!str2) {
        CU_FAIL("Cannot join two strings to a new one.");
        return;
    } // if
    ...
    str3 = qn_str_join("", srcs[0], srcs[1], srcs[2], QN_STR_ARG_END);
    if (!str3) {
        CU_FAIL("Cannot join three strings to a new one.");
        return;
    } // if
